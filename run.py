...
def run_dot_2d_astar():
    ...
    # Execute plan
    plan, cost = planner.plan()  # Assume `planner.plan()` returns both the path and the cost
    expanded_nodes = planner.get_expanded_nodes()
    num_expanded_paths = len(expanded_nodes)

    print(f"Path Cost: {cost}")
    print(f"Number of Expanded Paths: {num_expanded_paths}")
